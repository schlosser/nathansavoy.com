...
gulp.task("pdf", () =>
  gulp.src("src/pdf/**/*").pipe(plumber()).pipe(gulp.dest("./dist/pdf"))
);
...
gulp.task("watch", () => {
  ...
  gulp.watch("./src/pdf/**/*", gulp.series("pdf", "reload"));
...
gulp.task("deploy:rsync", (done) => {
  cp.exec("rsync -avuzh ./dist/* dan:/srv/nathansavoy.com/public_html/", () => {
